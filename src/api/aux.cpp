...
{
    boost::multiprecision::mpz_int res = static_cast<boost::
                                                         multiprecision::mpz_int>(input_string);
    return res;
}

boost::multiprecision::mpf_float parse_string_to_mpf_float(std::string const &input_string)
{
    boost::multiprecision::mpf_float res = static_cast<boost::
                                                           multiprecision::mpf_float>(input_string);
    return res;
}
                     ...
                     nlohmann::json &json_file)
{
    ...
    for (auto const &tkn : json_file["tokens"].items())
    {
        ...
        if (!tkn.value()["alias"].is_null())
            tokens.push_back(Token(tkn.key(), tkn.value()["alias"],
                                   tkn.value()["decimals"],
                                   external_price, normalize_priority));
        else
            tokens.push_back(Token(tkn.key(), "_NULL", tkn.value()["decimals"],
                                   external_price, normalize_priority));
        // TODO: Check if multiple tokens with "null" alias can create any issue
    ...
    for (auto const &o : json_file["orders"].items())
    {
        ...
        boost::multiprecision::mpz_int const sell_amount =
            parse_string_to_mpz_int(o.value()["sell_amount"]);

        boost::multiprecision::mpz_int const buy_amount =
            parse_string_to_mpz_int(o.value()["buy_amount"]);

        int const fee_token = _idx_tokens.at(o.value()["fee"]["token"]);

        boost::multiprecision::mpz_int const fee_amount =
            parse_string_to_mpz_int(o.value()["fee"]["amount"]);

        int const cost_token = _idx_tokens.at(o.value()["cost"]["token"]);

        boost::multiprecision::mpz_int const cost_amount =
            parse_string_to_mpz_int(o.value()["cost"]["amount"]);

        orders.push_back(Order(key, sell_token, buy_token, is_sell_order,
                               is_liquidity_order, allow_partial_fill,
                               sell_amount, buy_amount, fee_token,
                               fee_amount, cost_token, cost_amount));
    }
    ...
    for (auto const &am : json_file["amms"].items())
    {
        ...
        // We only use ConstantProduct AMMs, so this ensures that we ignore the rest
        ...
        boost::multiprecision::mpf_float const fee_fraction =
            parse_string_to_mpf_float(am.value()["fee"]);

        int const cost_token = _idx_tokens.at(am.value()["cost"]["token"]);

        boost::multiprecision::mpz_int const cost_amount =
            parse_string_to_mpz_int(am.value()["cost"]["amount"]);

        bool const is_mandatory = am.value()["mandatory"];

        amms.push_back(CP_AMM(key, pools_idx[0], pools_reserves[0],
                              pools_idx[1], pools_reserves[1], fee_fraction,
                              cost_token, cost_amount, is_mandatory));
    }
